<template>
  <EditApplication :loanApplication="loanApplication" :active="'details'">
...
let form = useForm({
  ...
  repayment_period: props.loanApplication.repayment_period,
});
...
selectMember({ target: { value: props.loanApplication.member_id } });
selectLoan({ target: { value: props.loanApplication.loan_type_id } });

function saveForm() {
  form.put(route("loanApplications.update", props.loanApplication.id), {
    onSuccess: () => {},
  });
}
</script>
